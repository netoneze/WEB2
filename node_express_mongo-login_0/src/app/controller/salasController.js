...
router.post("/contacted", async (req, res) => {
  ...
  const info = await mailer.sendMail({
    ...
    html:
      '<div style="padding: 10px; font-size: 18px;background-color: #f27830; color: #ffffff; border-radius: 5px; margin-left: 15px; margin-right: 15px;">' +
      formData.messageContact +
      '<br><br> <div style="font-size: 13px">Caso tenha alguma dúvida, favor entrar em contato com os e-mails: <br> lucascamacho@alunos.utfpr.edu.br <br> jaine@alunos.utfpr.edu.br <br> lucasmitsudo@gmail.com </div></div>', // plain text body
  });
...
router.patch("/update/:id", async (req, res) => {
  ...
  try {
    const updateSala = await Sala.updateOne(
      { _id: req.params.id },
...
router.patch("/update/code/:codSala", async (req, res) => {
  ...
  try {
    const sala = await Sala.findOne({ codigo: codSala });
    if (!sala) res.send("Sala não existe");
