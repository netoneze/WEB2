...
import {
  ...
  IUser,
  IUserQuestion,
  IUserQuestionAnswers,
} from './../../../services/exercicios.interface';
...
export class ContentComponent implements OnInit {
  ...
  async ngOnInit() {
    this.user;
    this.auxQuestion;
    this.questions = (await (await this.userQuestionService.getQuestion())
      .pipe(take(1))
      .toPromise()) as IQuestion[];
    this.user = this.authService.getUser();
    this.auxQuestion = this.questions[0];

    if (this.user._id)
      this.userQuestion = await this.verifyUserQuestioAlreadyExist(
        this.user._id
      );

    this.questions.forEach((f) => {
      const hasAnswered = this.hasAswersFiveTimes(f._id ?? '');
  ...
  hasAswersFiveTimes(id: string) {
    const res = this.userQuestion.map((f) => f.questions);
    const hasQuestion = res.map((f) => f.find((m) => m.qid === id));

    if (hasQuestion) {
      const qtd = hasQuestion.map((m) => m?.answers.length);

      return qtd[0] ? qtd[0] > 4 : false;
    }
  ...
  goToNext() {
    ...
    this.has = this.hasAswersFiveTimes(this.auxQuestion._id ?? '');
    while (this.has) {
      this.has = this.hasAswersFiveTimes(this.auxQuestion._id ?? '');
    }
    if (!this.has) this.next();
  }
  ...
  next() {
    if (this.i == this.questions.length) {
      this.i = 0;
      this.showExec = false;
      this.snackBar.open('Você respondeu todas as perguntas', '', {
        duration: 2000,
      });
    }
  }
  ...
  async verifyUserQuestioAlreadyExist(id: string): Promise<IUserQuestion[]> {
    return (await (await this.userQuestionService.getUserQuestionByUserId(id))
      .pipe(take(1))
      .toPromise()) as IUserQuestion[];
  }

  async responder() {
    if (this.userQuestion.length > 0) {
      const question = this.userQuestion.map((f) =>
        f.questions.find((q) => q.qid == this.auxQuestion._id)
      );
      ...
      const res = (await (
        await this.userQuestionService.updateAnswers(
          this.userQuestion[0],
          this.userQuestion[0]?._id
        )
      )
        .pipe(take(1))
        .toPromise()) as IResPut;
    } else {
  ...
  async update(question: {
    ...
    const index = this.userQuestion[0].questions.findIndex(
      (f) => f.qid == question.qid
    );
  ...
  async createUser(): Promise<IUserQuestion> {
    ...
    if (this.user._id) userQ.userid = this.user._id;
    ...
    return (await (await this.userQuestionService.createUserQuestion(userQ))
      .pipe(take(1))
      .toPromise()) as IUserQuestion;
  }
