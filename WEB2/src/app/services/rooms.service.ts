...
import { IIdAluno } from '../components/dialogs/sala-dialog/sala-dialog.component';
...
export class RoomsService {
  ...
  updateSalaAlunosById(codSala: string, idAluno: IIdAluno) {
    const stringify = JSON.stringify(idAluno);
    const idJson = JSON.parse(stringify);

    return this.http.patch('update/code', codSala, idJson);
  }
}
